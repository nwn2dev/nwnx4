...
extern "C"
{
	...
	/// Executed by NWScript function `RetrieveCampaignObject`, before `GetGFF`. Since NWN2 must
	/// manage the memory containing the GFF data, this function must return the size of the buffer
	/// to allocate for the result parameter of `GetGFF`
	/// @return the size of the buffer NWN2 needs to allocate for receiving the GFF data
	__declspec(dllexport) size_t
	  __cdecl NWNXCPlugin_GetGFFSize(void* cplugin, const char* sVarName);

	/// Executed by NWScript function `RetrieveCampaignObject`, after GetGFFSize
	/// @param cplugin User data pointer, as returned by `NWNXCPlugin_New`
